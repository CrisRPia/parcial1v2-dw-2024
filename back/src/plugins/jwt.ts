...
export default fp<FastifyJWTOptions>(async (fastify) => {
        ...
        async function (request: FastifyRequest, reply: FastifyReply) {
            ...
            if (!usuarioToken.is_admin)
                throw reply.unauthorized(
                    "Tienes que ser admin para hacer eso."
                );
        }
    );
    ...
    fastify.decorate(
        ...
        async function (request: FastifyRequest, reply: FastifyReply) {
            ...
            const id_usuario = Number(
                (request.params as IdUsuarioType).id_usuario
            );
    ...
    fastify.decorate(
        ...
        async function (request: FastifyRequest, reply: FastifyReply) {
            ...
            if (
                ...
                usuarioToken.id_usuario !== id_usuario
            )
                throw reply.unauthorized(
    ...
    fastify.decorate(
        ...
        async function (request: FastifyRequest, reply: FastifyReply) {
            ...
            if (
                usuarioToken.id_usuario !== tema.id_usuario &&
                !usuarioToken.is_admin
            )
                throw reply.unauthorized(
